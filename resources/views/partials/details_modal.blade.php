<div id="detailsModal" x-cloak class="hidden fixed inset-0 z-50 overflow-y-auto">
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" onclick="closeDetailsModal()"></div>

    <div class="relative min-h-screen flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg shadow-2xl p-6 border border-gray-100 dark:border-gray-700 relative animate-fade-in-up">
    ...
    window.openDetailsModal = function(request) {
        try {
            ...
            const getStatusBadgeClass = (status) => {
                ...
                if (st.includes('pending')) return 'bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800/40';
                if (st === 'approved') return 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800/40';
            ...
            document.getElementById('detailName').textContent = (request.first_name || '') + ' ' + (request.last_name || '') + (request.suffix ? ' ' + request.suffix : '');
