...
<head>
    ...
    <title>Reset Password - {{ $global_settings['system_short_name'] ?? 'IFI CMS' }}</title>
        ...
        <div
            ...
            <div class="w-full max-w-md mx-auto">
                <div class="mb-12">
                    ...
                    <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-3 tracking-tight">Reset Password</h2>
                    <p class="text-base text-gray-500 dark:text-gray-400">Create a new password for your account.</p>
                </div>
                ...
                <form action="{{ route('password.update') }}" method="POST" class="space-y-6">
                    @csrf
                    <!-- Pass the email silently from session -->
                    <input type="hidden" name="email" value="{{ $email }}">

                    <div class="relative group">
                        <label
                            class="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-1.5 uppercase tracking-wider group-focus-within:text-primary dark:group-focus-within:text-primary transition-colors">New
                            Password</label>
                        <div class="relative">
                            <i
                    ...
                    <div class="relative group">
                        <label
                            class="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-1.5 uppercase tracking-wider group-focus-within:text-primary dark:group-focus-within:text-primary transition-colors">Confirm
                            Password</label>
                        <div class="relative">
                            <i
                                class="fas fa-lock absolute left-5 top-1/2 transform -translate-y-1/2 text-gray-300 dark:text-gray-600 text-lg transition-colors group-focus-within:text-primary"></i>
